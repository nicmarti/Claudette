...
3. **Verify** by calling `list_graph_stats` again and report the results:
...
- The graph is stored as a SQLite database (`.claudette/graph.db`) in the repo root
- Binary files, generated files, and patterns in `.claudetteignore` are skipped
- Supported languages: Python, TypeScript/JavaScript, Go (more coming incrementally)
